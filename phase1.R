library(readr)
Consumer_Complaints <- read_csv("C:/Users/nvidi/Desktop/Lambton/Consumer_Complaints.csv")
...
write.csv(Consumer_Complaints, file = "C:/Users/nvidi/Desktop/Lambton/cleaned.csv")
...
#Consumer_Complaints = Consumer_Complaints[!is.na(Consumer_Complaints$State) & !is.na(Consumer_Complaints$ZIP.code), ]
#Consumer_Complaints = Consumer_Complaints[!is.na(Consumer_Complaints$ZIP.code), ]

#removing the rows where state is NA
naremove = is.na(Consumer_Complaints$State)
testvar = which(naremove==c("TRUE"))
Consumer_Complaints = Consumer_Complaints[-testvar, ]

#removing the rows where zip code is NA
naremove1 = is.na(Consumer_Complaints$ZIP.code)
testvar2 = which(naremove1==("TRUE"))
Consumer_Complaints = Consumer_Complaints[-testvar2, ]


##checking for duplicate entries in the dataset and if yes then printing the statement
...
#to remove duplicacy
Consumer_Complaints = Consumer_Complaints[!duplicated(Consumer_Complaints$Complaint.ID), ]
...
ggplot(cc_response,aes(x=Company.response.to.consumer,width=0.5)) + geom_bar(stat="identity",aes(y=Complaint.ID/sum(Complaint.ID)*100))+ geom_text(aes(label=paste(round(Complaint.ID/sum(Complaint.ID)*100,2),"%"),y=Complaint.ID/sum(Complaint.ID)*100),vjust=-0.5,check_overlap = TRUE,size=3) + xlab(label="Company response") + ylab(label="Percentage of complaints") + theme(axis.text.x=element_text(angle = 90, hjust = 1,vjust = 0.5))
...
ggplot(cc,aes(ComplaintYear, fill = ComplaintYear)) + geom_bar(position = "dodge") + xlab(label="Complaint Year") + ylab(labe="Number of Complaints")
...
ggplot(cc_medium,aes(x=Submitted.via,fill = Submitted.via,width=0.5)) + geom_bar(stat="identity",aes(y=Complaint.ID/sum(Complaint.ID)*100)) + geom_text(aes(label=paste(round(Complaint.ID/sum(Complaint.ID)*100,2),"%"),y=Complaint.ID/sum(Complaint.ID)*100),vjust=-0.5,check_overlap = TRUE,size=3) +  ylab(label="Percentage of complaints") + xlab(label="Submitted via medium")
